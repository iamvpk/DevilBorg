...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**[Devil's](tg://need_update_for_some_feature/) CPU Model:**\n"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**[Devil's](tg://need_update_for_some_feature/) SUICIDE BOMB:**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:	
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**[Devil's](tg://need_update_for_some_feature/) STDPLUGINS:**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:	
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**[Devil's](tg://need_update_for_some_feature/) PIP UPGRADE:**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:	
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**[Devil's](tg://need_update_for_some_feature/) Date & Time Of New York:**\n\n\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:	
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**[Devil's](tg://need_update_for_some_feature/) Environment Module:**\n\n\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:	
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**[Devil's](tg://need_update_for_some_feature/) Neofetch Installed, Use `.sysd` :**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**[Devil's](tg://need_update_for_some_feature/) Telethon Updated**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**[Devil's](tg://need_update_for_some_feature/) , Server Speed Calculated:**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**[Devil's](tg://need_update_for_some_feature/) , Coffeehouse Updated:**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**[Devil's](tg://need_update_for_some_feature/) , Sex Installed To DevilBorg**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**[Devil's](tg://need_update_for_some_feature/) , Telegram Installed To DevilBorg**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
...
#    if dirname == tempdir:
    ...
    OUTPUT = f"**[Devil's](tg://need_update_for_some_feature/) , PIP Installed To Your DevilBorg...**\n"
    stdout, stderr = await process.communicate()
    if len(stdout) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(stdout)) as out_file:
            out_file.name = "exec.text"
